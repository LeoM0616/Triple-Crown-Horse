"""
芝・中長距離専用モデル 最終シミュレーター
- 当日バイアス補正（外枠不利を ±7% で強め補正）
...
def calc_day_bias_long(df_raw: pd.DataFrame, race_date: str, venue_code: str) -> dict:
    """
    長距離版当日バイアス補正: 外枠ほど距離ロスが大きいため補正強度 ±7%
    """
    bias = {'bracket_bias': {}}
    same_day = df_raw[
    ...
    if len(same_day) > 0:
        ...
        for b, avg in bracket_avg.items():
            # 長距離: ±7% 補正（短距離の ±5% より精密化）
            bias['bracket_bias'][int(b)] = np.clip(
                (overall_avg - avg) / (overall_avg + 1e-5) * 0.14, -0.07, 0.07
            )
    return bias
...
def simulate_turf_long():
    ...
    # モデルロード
    model     = joblib.load(os.path.join(model_dir, 'lgbm_turf_long_final.pkl'))
    meta      = joblib.load(os.path.join(model_dir, 'lgbm_turf_long_final_meta.pkl'))
    ...
    for ev_thresh in ev_thresholds:
        ...
        for race_id, group in sub_2025.groupby('race_id'):
            ...
            day_bias   = calc_day_bias_long(df_raw, race_date, venue_code)
    ...
    # ─── 出力 ─────────────────────────────────────────────────
    print(f"\n{'='*72}")
    print(f"  🏇 最終収支報告書（芝・中長距離専用AI）2025年テストデータ")
    print(f"  ※ オッズ除外学習 + 騎手×競馬場TE + 当日バイアス補正(±7%) 適用済み")
    print(f"{'='*72}")
    ...
    best = next(r for r in sweep_results if r['ev'] == best_ev)
